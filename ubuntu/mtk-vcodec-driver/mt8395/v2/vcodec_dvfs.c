...
/**
 * free_next_hist - free next codec_history and shift the next one after next
 *                  forward (not used)
 *
 ...
 *                   is really for the next one after next job.
 *
...
long long est_next_submit(struct codec_history *hist)
{
	...
	/* Add 2x estimated gap for the next one after next job */
	if (hist->cur_cnt == 1)
