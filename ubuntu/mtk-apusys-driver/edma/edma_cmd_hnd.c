...
irqreturn_t edma_isr_handler(int irq, void *edma_sub_info)
{
	...
	edma_set_reg32(edma_sub->base_addr, 0x814, 0x1);
...
{
	if (desp_iommu_en)
		edma_set_reg32(base_addr, 0x004, ((0x2 << 30) | (0x2 << 28)));

	udelay(5);

	edma_write_reg32(base_addr, 0x800, ext_addr);

	/* [28] irq_en, [20] vc_sw_rst, [19:0] num */
	edma_write_reg32(base_addr, 0x804,
			 ((0x1 << 28) | ((num_desp << 0) & 0xfffff)));

	/* [16] stop, [0] fire */
	edma_write_reg32(base_addr, 0x808, (0x1 << 0));
}
...
{
	struct edma_device *edma_device = container_of(power_timer,
						       struct edma_device,
						       power_timer);
