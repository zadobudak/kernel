			       ...
			       enum dvfs_device_id dvfs_dev_id)
{
	...
	if (dvfs_dev_id < DVFS_DEV_MVPU || dvfs_dev_id >= DVFS_DEV_MAX)
		return -1;
...
#else
static int mt8188_apu_real_power(struct devfreq *devfreq, unsigned int *power,
...
{
	enum dvfs_device_id dvfs_dev_id = DVFS_DEV_MVPU;
	struct apu_pwr_devfreq_st *apu_pwr_devfreq_ptr = NULL;
