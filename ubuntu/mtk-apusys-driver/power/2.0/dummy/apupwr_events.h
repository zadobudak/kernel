...
TRACE_EVENT(apupwr_pwr,
	...
	unsigned int vsram, unsigned int vpu0_freq, unsigned int vpu1_freq,
	unsigned int mdla0_freq, unsigned int mdla1_freq,
	unsigned int conn_freq, unsigned int ipuif_freq,
	unsigned long long id),
	TP_ARGS(vvpu, vmdla, vcore, vsram, vpu0_freq, vpu1_freq,
	mdla0_freq, mdla1_freq, conn_freq, ipuif_freq, id),
	TP_STRUCT__entry(
	...
	TP_fast_assign(
		...
		__entry->mdla1_freq = mdla1_freq;
		__entry->conn_freq = conn_freq;
		__entry->ipuif_freq = ipuif_freq;
		__entry->id = id;
	),
	TP_printk(
		"V_vpu=%d,V_mdla=%d,V_core=%d,V_sram=%d,F_v0=%d,F_v1=%d,F_m0=%d,F_m1=%d,F_conn=%d,F_ipuif=%d,time_id=%llu",
		__entry->vvpu,
		__entry->vmdla,
		__entry->vcore,
		__entry->vsram,
...
TRACE_EVENT(apupwr_rpc,
	...
	unsigned int mdla0_cg_stat, unsigned int mdla1_cg_stat),
	TP_ARGS(spm_wakeup, rpc_intf_rdy, vcore_cg_stat, conn_cg_stat,
	vpu0_cg_stat, vpu1_cg_stat, mdla0_cg_stat, mdla1_cg_stat),
	TP_STRUCT__entry(
	...
	TP_fast_assign(
		...
		__entry->mdla0_cg_stat = mdla1_cg_stat;
	),
	TP_printk(
		"spm_wakeup=0x%x,rpc_intf_rdy=0x%x,vcore_cg=0x%x,conn_cg=0x%x,v0_cg=0x%x,v1_cg=0x%x,m0_cg=0x%x,m1_cg=0x%x",
		__entry->spm_wakeup,
