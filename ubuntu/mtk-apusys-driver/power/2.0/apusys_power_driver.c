...
int32_t apu_user_reset(enum DVFS_USER user)
{
	return hal_config_power(PWR_CMD_USER_RESET, user, NULL);
}
EXPORT_SYMBOL(apu_user_reset);
...
static int apu_power_probe(struct platform_device *pdev)
{
	...
	ret = init_devfreq_cooling(&pdev->dev);
	if (ret)
		return ret;
...
//#if IS_ENABLED(CONFIG_FPGA_EARLY_PORTING)
	...
	platform_set_drvdata(pdev, init_power_data.dev);
	return 0;
