...
int vpu_lk(struct vpu_device *vpu_device, struct platform_device *pdev)
{
	...
	vpu_drv_debug("mblk:[0x%x:0x%x] | head:0x%x | alg: 0x%x | pre: 0x%x\n",
	...
	*pIramNum = 0;
	...
	for (i = 0 ; i < num ; i++) {
		...
		vpu_drv_debug("copy %s header(0x%X) to 0x%x\n",
